...
import com.cmms.logistics.dto.VehicleInventoryResponseDto;
...
public interface VehicleInventoryClients {
    ...
    ResponseEntity<VehicleInventoryResponseDto> getQualityVehicleInventory(@PathVariable("id") long id);

    @PutMapping("/production/vehicleInventory/{id}/status")
    ResponseEntity<Void> updateVehicleStatus(@PathVariable("id") long id, @RequestParam("status") String status);
